...
var Page = (function($, App) {
    ...
    function connectPin() {
        ...
        if (((pin.length == 4) || (pin.length == 8 && checkWPSPINCode(pin))) && !pin_already_used) {
            ...
            Messages.error((!pin_already_used) ? 'Por favor informe um PIN.' : 'PIN já usado na última sessão.<br/>Por favor informe um PIN.');
        }
    }
    ...
    function setupUi() {
            ...
            if (this.id == "WLANInterface_1_WPSEnable") {
                if ( id.attr("state") == 0) {
                    ...
                    if ( beaconValue == "Basic" || beaconValue == "WPA" ) {
                        Messages.error('Função WPS não pode ser habilitada em modo de segurança WEP ou WPA.');
                        return;
                    }
                    if ($("#WLANInterface_1_Config_HideSSID").attr("state") == 0) {
                        if (! confirm('Isto também irá habilitar o anúncio do SSID, você deseja continuar ?')) { 
                            return;
                        } else {
            ...
            if (this.id == "WLANInterface_1_WMMEnable") {
                if (id.attr("state") == 1) {
                    if ($("#WLANConfig_Standard").val() == "11n") {
                        Messages.error('Função WMM não pode ser desabilitada em modo de operação 802.11n.');
                        return;
                    } else if ($("#WLANConfig_Standard").val() == "11bgn" || $("#WLANConfig_Standard").val() == "Auto") {
                        if (! confirm('Não é recomendado desabilitar função WMM em modo de operação 802.11bgn, você deseja continuar ?')) { 
                            return;
                        }
                    }
                }
        ...
        $("#WLANConfig_Standard").bind("change", function() {
            ...
            if( standard == "11n" || standard == "11gn" || standard == "11bgn" || standard == "Auto" ) {
                if ( beaconValue == "WPA" || beaconValue == "Basic") {
                    if (! confirm('Isto também irá alterar o modo de segurança, você deseja continuar ?')) {
        ...
        $("#cancelBas").bind("click", function() {
            ...
            cleanupUpdates();
            if (Object.keys(updatesAdv).length != 0) {
                if (! confirm('Isto também irá apagar toda personalização efetuada na configuração avançada do Wi-Fi, você deseja continuar ?')) {
                    return;
                }
            }
            updatesBas = {};
        ...
        $("#cancelAdv").bind("click", function() {
            ...
            cleanupUpdates();
            if (Object.keys(updatesBas).length != 0) {
                if (! confirm('Isto também irá apagar toda personalização efetuada na configuração basica do Wi-Fi, você deseja continuar ?')) {
                    return;
                }
            }
            updatesBas = {};
    ...
    function switchWifiConfigCheck(cur_tab_idx, next_tab_idx) {
        ...
        if ( cur_tab_idx !=  next_tab_idx ) {
            ...
            if ( cur_tab_idx == "01" ) {
                if(!validator.validate()) { // only validate SSID and password
                    Messages.error('Por favor valide a configuração ou cancele.');
                    ret = false;
                }
            }
        }
